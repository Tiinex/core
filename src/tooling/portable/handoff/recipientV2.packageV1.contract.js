...
export function parseHandoffPackageV1(markdown = '') {
  ...
  return Object.freeze({
    ...
    carrierDimension: field(sectionText(body, 'Carrier Continuity'), 'Carrier Dimension'), parentCarrierDimension: field(sectionText(body, 'Carrier Continuity'), 'Parent Carrier Dimension'), parentPackageSha256: String(field(sectionText(body, 'Carrier Continuity'), 'Parent Package SHA256') || '').trim().toLowerCase(), parentPackageFilename: field(sectionText(body, 'Carrier Continuity'), 'Parent Package Filename'), carrierCheckpoint: field(sectionText(body, 'Carrier Continuity'), 'Carrier Checkpoint'), majorReason: field(sectionText(body, 'Carrier Continuity'), 'Major Reason'), carrierProfileId: normalizeProfileId(field(sectionText(body, 'Carrier Continuity'), 'Carrier Profile Id')), requiredMajorWorkspaceIds: normalizeWorkspaceIds(field(sectionText(body, 'Carrier Continuity'), 'Required Major Workspace Ids')),
...
export function validatePackageFields(value, findings) {
  ...
  if (value?.parentPackageSha256 && !/^[a-f0-9]{64}$/.test(String(value.parentPackageSha256 || ''))) findings.push(finding('error', 'portable.handoff-package-v1.parent-package-sha256-invalid', 'Parent Package SHA256 must be a 64-hex SHA-256 when declared.', { value: value.parentPackageSha256 || '' }));
  if (value?.parentPackageFilename && /[\\/]/.test(String(value.parentPackageFilename || ''))) findings.push(finding('error', 'portable.handoff-package-v1.parent-package-filename-invalid', 'Parent Package Filename must be a basename when declared.', { value: value.parentPackageFilename || '' }));
